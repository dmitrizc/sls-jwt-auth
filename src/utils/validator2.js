...
// Validate stringified JSON body, validates with validateToPlainErrors and return selected fields and error
Joi.__proto__.validateRequestAndSelect = function (body, schema, fields) {
  const request = _pick(JSON.parse(body), fields);
  const errors = Joi.validateToPlainErrors(request, _pick(schema, fields));
